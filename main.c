...
int main(int argc, char** argv) {
    ...
    if (procID == root) {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (eq_pointer) {
                ...
                if (!strcmp(parameterName, "-q")) 
                    params[0] = value;
                else if (!strcmp(parameterName, "-p"))
                    params[1] = value;
    ...
    int numPoints_rank; // Quantidade de pontos do rank

    // Distribui as quantidades de pontos para cada rank
    MPI_Scatter(sendBuf_counts, 1, MPI_INT,
                &numPoints_rank, 1, MPI_INT,
                root, MPI_COMM_WORLD);

    int localSize_Q = numPoints_rank * params[2]; // Número de elementos do conjunto Q do rank
    int localSize_R = numPoints_rank * params[3]; // Número de elementos do conjunto R do rank
